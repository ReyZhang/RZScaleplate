...
-(void)layoutBorder {
    ...
    if (self.oritation==Horizontal) {
        ...
        _lineLayer.frame=CGRectMake(2,_topLayer.frame.size.width,kMaxLineLength+5, 1);
        [_linePath moveToPoint:_lineLayer.frame.origin];
...
-(void)settingsScrollView {
    if(self.oritation==Horizontal) {
        ...
        self.scrollView.contentInset=UIEdgeInsetsMake(self.frame.size.height/2, 0, self.frame.size.height/2, 0);
        
        
        if (self.value) {
            CGFloat offsetY= 5*((self.maxValue-[_value floatValue])-self.miniValue)/self.stepValue-self.frame.size.height/2;     ////////由于取值时，为使数据从大到小排列，用最大值self.maxValue减去取的值得到的结果，在反向时需要再反向回来  rey
            self.scrollView.contentOffset=CGPointMake(self.scrollView.contentOffset.x,offsetY);
            
        }else {
            self.scrollView.contentOffset=CGPointMake( 0,-self.frame.size.height/2);
        }
